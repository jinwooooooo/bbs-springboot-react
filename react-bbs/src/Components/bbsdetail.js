...
function Bbsdetail(){
    ...
    const fetchData = async (s) => {
        await axios.get("http://localhost:3000/getDetailPage", { params:{ "seq":s, "userId":userId }})
            .then(function(resp){
    ...
    const fetchComment = async (s) => {
        await axios.get("http://localhost:3000/getComment", { params:{ "seq":s }})
            .then(function(resp){
               //console.log(resp.data);
    ...
    const insertData = async (u, c, d) => {
        await axios.get("http://localhost:3000/writeComment", { params:{ "id":u, "content":c, "detailSeq":d} })
        .then(function(resp){
            if(resp.data === "OK"){
             if(!alert('코멘트 작성 완료!')){
                fetchComment(seq);
    ...
    function Detaildelete(){
        ...
        if(sessionStorage.getItem("user") === bbsDetail.id){
            return(
                <tr>
                    <td colSpan="2" align="center">
